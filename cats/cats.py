...
def about(subject):
    ...
    def return_function(w):
        ws = split(remove_punctuation(lower(w)))
        for word in subject:
            for w in ws:
                if w == word:
                    return True
...
def accuracy(typed, source):
    ...
    if len(typed_words) != 0 and len(source_words) != 0:
        acc = num_matched / len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    ws = [word for word in word_list if diff_function(typed_word, word, limit) <= limit]
    if len(ws) == 0 or typed_word in word_list:
        return typed_word
    else:
        return min(ws, key = lambda x: diff_function(typed_word,x,limit))
    # END PROBLEM 5


def feline_fixes(typed, source, limit, cnt = 0):
    ...
    if typed[0] == source[0]:
        return feline_fixes(typed[1:], source[1:], limit, cnt)
    else:
        cnt += 1
        return 1 + feline_fixes(typed[1:], source[1:], limit, cnt)
    # END PROBLEM 6
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    result = [[] for i in range(len(get_all_times(match)))]
    for word_index in word_indices:
        fastest = time(match, 0, word_index)
        fastest_player = 0
        for player_index in player_indices:
            if time(match, player_index, word_index) < fastest:
                fastest_player = player_index
                fastest = time(match, player_index, word_index)
        result[fastest_player].append(get_word(match, word_index))
    return result
    # END PROBLEM 10
