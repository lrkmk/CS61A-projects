...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    onep = player_score%10
    teno = (opponent_score//10)%10
    result = 3*abs(onep-teno)
    if result>1:
        return result
    return 1
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    results = [averaged_roll_dice(i, dice) for i in range(1, 11)]
    return max(enumerate(results),key=lambda x: x[1])[0]+1
    # END PROBLEM 9
